...
$function  = $input->getCmd('function', 'jSelectVirtualdomain');
$listOrder = $this->escape($this->state->get('list.ordering'));
$listDirn  = $this->escape($this->state->get('list.direction'));
?>
<form action="<?php echo JRoute::_('index.php?option=com_virtualdomains&view=virtualdomains&layout=modal&tmpl=component&function='.$function);?>" method="post" name="adminForm" id="adminForm" class="form-inline">
		...
		<thead>
			<tr>				
					...
					<?php echo JHTML::_('grid.sort', 'Domain', 'a.domain', $listDirn, $listOrder ); ?>
								...
								<th class="title">
					<?php echo JHTML::_('grid.sort', 'Template', 'a.template', $listDirn, $listOrder ); ?>
				</th>
								<th class="title">
		...
		<?php foreach ($this->items as $i => $item) : ?>
			<tr class="row<?php  echo $i % 2; ?>">
					...
					<a class="pointer" onclick="if (window.parent) window.parent.<?php echo $this->escape($function);?>('<?php echo $item->id; ?>', '<?php echo $this->escape(addslashes($item->name)); ?>');">
						<?php echo $this->escape($item->domain); ?></a>
				</td>
